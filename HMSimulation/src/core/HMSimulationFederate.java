...
public class HMSimulationFederate extends SEEAbstractFederate implements Observer {

	private static final int MAX_WAIT_TIME = 10000;
	
	
	private HMSpaceStation hmSpaceStation = null;
	private Astronaut_HM astronaut_hm = null;
	...
	public HMSimulationFederate(SEEAbstractFederateAmbassador seefedamb, HMSpaceStation lunarRover, Astronaut_HM Astronaut_HM) {
		super(seefedamb);
		this.hmSpaceStation  = lunarRover;
		this.astronaut_hm  = Astronaut_HM;
	...
	public void configureAndStart(Configuration config) throws ConnectionFailed, InvalidLocalSettingsDesignator, UnsupportedCallbackModel, 
		...
		if(!SynchronizationPoint.INITIALIZATION_STARTED.isAnnounced()){
			 ...
			 */
			super.publishElement(hmSpaceStation, "HMSpaceStation");
			super.publishElement(astronaut_hm, "Astronaut_HM");
			super.subscribeReferenceFrame(FrameType.EarthMoonL2Rotating);
		...
		// State machine to control the federate's actions
		statemachine:
		while(true)
		{
			String nxt = scn.next();
			
			switch (nxt.toLowerCase())
			{
			case "w":
				this.astronaut_hm.startSpaceWalk(this.getTime().getFederateTimeCycle());
				break;
			case "x":
	...
	protected void doAction() {
		...
		Position curr_pos = this.hmSpaceStation.getPosition();
		Position curr_pos_astr = this.astronaut_hm.getPosition();
		
		this.hmSpaceStation.updatePosition(this.getTime().getFederateTimeCycle());
		this.astronaut_hm.updatePosition(this.getTime().getFederateTimeCycle());

		try {
			super.updateElement(this.hmSpaceStation);
			super.updateElement(this.astronaut_hm);
			
			System.out.println("SpaceStation: " + curr_pos);
			System.out.println("Astronaut: " + curr_pos_astr);
